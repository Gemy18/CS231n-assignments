...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    h0,cashe1=affine_forward(features,W_proj,b_proj)
    ...
    h_out, dW_vocab, db_vocab = temporal_affine_backward(dout, cache4)

    if self.cell_type == 'rnn':
      dx, dh0, dWx, dWh, db = rnn_backward(h_out, cache3)
    else:
      dx, dh0, dWx, dWh, db = lstm_backward(h_out, cache3)


    dW_embed = word_embedding_backward(dx, cache2)
    dfeatures, dW_proj, db_proj = affine_backward(dh0, cashe1)
  ...
  def sample(self, features, max_length=30):
    ...
    # a loop.                                                                 #
    ###########################################################################
    # initial hiddel layer calculation
    h, _ = affine_forward(features, W_proj, b_proj)
    c = np.zeros(h.shape)


    # intial input is <START> token
