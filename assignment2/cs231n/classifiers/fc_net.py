...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    delta1, dW1, db1=affine_relu_backward(delta2,cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(self.num_layers):
      self.params['W'+str(i+1)]=np.random.normal(scale=weight_scale, size=(all_dims[i], all_dims[i+1]))
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if i == self.num_layers - 1:
        ...
        if self.use_batchnorm:
          h, cache_h = affine_batch_rule_forward(h, w, b,gamma,beta,bn_param)

        else:
          h, cache_h = affine_relu_forward(h, w, b)
        if self.use_dropout:
          h, dropout_cashe=dropout_forward(h,  self.dropout_param)
          hidden['dropout-cache_h'+str(i+1)] = dropout_cashe
      hidden['h' + str(i + 1)] = h
    ...
    for i in range(self.num_layers)[::-1]:
      dh = hidden['dh' + str(i+1)]
      cashe=hidden['cache_h'+ str(i+1)]

      if i == self.num_layers-1:
        dh, dw, db = affine_backward(dh, cashe)
        hidden['dh' + str(i)] = dh
        hidden['dW' + str(i+1)] = dw
        hidden['db' + str(i+1)] = db

      else :
        if self.use_dropout:
          dropout_cashe = hidden['dropout-cache_h' + str(i + 1)]
          dh=dropout_backward(dh,dropout_cashe)

        if self.use_batchnorm:
          dh, dw, db, dgamma, dbeta = affine_norm_relu_backward(dh, cashe)
          hidden['dh' + str(i)] = dh
          hidden['dW' + str(i+1)] = dw
          hidden['db' + str(i+1)] = db
          hidden['dgamma' + str(i+1)] = dgamma
          hidden['dbeta' + str(i+1)] = dbeta
        else :
          dh, dw, db = affine_relu_backward(dh, cashe)
          hidden['dh' + str(i)] = dh
          hidden['dW' + str(i + 1)] = dw
          hidden['db' + str(i + 1)] = db
