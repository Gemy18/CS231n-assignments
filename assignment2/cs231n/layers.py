    ...
    for depth in xrange(F):
      for r in xrange(0,H,S):
        for c in xrange(0,W,S):
          a=np.sum(x_pad[n, :, r:r + HH, c:c + WW] * w[depth, :, :, :]) + b[depth]
          out[n, depth, r / S, c / S] = a
...
def conv_backward_naive(dout, cache):
  ...
  for n in xrange(N):
    for depth in xrange(F):
      for r in xrange(0,H,S):
        for c in xrange(0,W,S):
          dw[depth, :, :, :] += x_pad[n, :, r:r + HH, c:c + WW] * dout[n, depth, r/S,c/S]
          dx[n,:,r:r+HH,c:c+WW] += w[depth,:,:,:] * dout[n,depth,r/S,c/S]

  paddingrows = range(P) + range(H + P, H + 2 * P, 1)
  paddingcolumns = range(P) + range(W + P, W + 2 * P, 1)
  dx = np.delete(dx, paddingrows, axis=2)
  dx = np.delete(dx, paddingcolumns, axis=3)
...
def max_pool_forward_naive(x, pool_param):
  ...
  for n in xrange(N):
    for i in xrange(C):
      for r in xrange(0,H,S):
        for c in xrange(0,W,S):
          out[n,i,r/S,c/S] = np.max(x[n,i,r:r+pool_h,c:c+pool_w])
    ...
    for depth in xrange(C):
      for r in xrange(OUT_H):
        for c in xrange(OUT_W):
          x_pool = x[n, depth, r * S:r * S + pool_h, c * S:c * S + pool_w]
          mask = (x_pool == np.max(x_pool))
          dx[n, depth, r * S:r * S + pool_h, c * S:c * S + pool_w] += mask * dout[n, depth, r, c]
